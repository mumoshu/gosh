...
func (s *server) RouteChat(stream pb.RouteGuide_RouteChatServer) error {
	...
	go func() {
		...
		for s.Scan() {
			...
			if err := stream.Send(&pb.RouteNote{Message: s.Text()}); err != nil {
	...
	f := func() bool {
		return st.err.done && st.recv.done && st.out.done && st.run.done
	}
...
func newServer() *server {
	s := &server{routeNotes: make(map[string][]*pb.RouteNote)}
